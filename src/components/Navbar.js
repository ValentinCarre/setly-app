...
export default function Navbar() {
  ...
  async function refreshUnread(uid) {
    ...
    if (!id) return;
    const { count } = await supabase.from('messages').select('*', { count: 'exact', head: true }).eq('receiver_id', id).eq('read', false);
  ...
  useEffect(() => {
    ...
    // Listen for localStorage signal from messages page
    function onStorage() {
      const ts = localStorage.getItem('setly-messages-read');
      if (ts) refreshUnread();
    }
    window.addEventListener('storage', onStorage);
 
    // Also check periodically for same-tab updates via localStorage timestamp
    const checkInterval = setInterval(() => {
      const ts = localStorage.getItem('setly-messages-read');
      if (ts && Date.now() - parseInt(ts) < 3000) {
        refreshUnread();
      }
    }, 1000);
  ...
  const handleLogout = async () => {
    ...
    setUser(null); setProfile(null); setDisplayName(''); setUnreadCount(0); setMenuOpen(false);
      ...
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          ...
          {user ? (
              ...
              <div className="relative">
                ...
                {menuOpen && (
                  <div className="absolute right-0 top-12 w-56 bg-bg-card border border-border rounded-xl p-2 animate-fade-up shadow-xl">
                    <Link href="/profil" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-muted hover:bg-bg-hover hover:text-white transition"><span>👤</span> Mon profil</Link>
                    <Link href="/messages" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-muted hover:bg-bg-hover hover:text-white transition">
                      <span>💬</span> Messages
            ...
            <div className="flex items-center gap-3">
              <Link href="/login" className="text-sm text-muted border border-border px-4 py-2 rounded-lg hover:text-white hover:border-muted transition hidden sm:flex items-center gap-2"><span>👤</span> Connexion</Link>
