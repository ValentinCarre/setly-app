...
export default function Profil(){const[user,setUser]=useState(null);const[profile,setProfile]=useState(null);const[data,setData]=useState(null);const[loading,setLoading]=useState(true);const[pendingRatings,setPendingRatings]=useState(null);const[ratingsLoaded,setRatingsLoaded]=useState(false);const router=useRouter();const supabase=createClient();
...
function ArtistDashboard({data,user,completion,supabase}){
  ...
  return(
    <div className="min-h-screen px-4 py-16 sm:py-20"><div className="max-w-2xl mx-auto">
      ...
      {/* UPCOMING */}
      {(()=>{const todayA=new Date().toISOString().split('T')[0];const upcoming=accepted.filter(d=>d.soirees?.date_soiree>=todayA);const past=accepted.filter(d=>d.soirees?.date_soiree<todayA);const mn=['Jan','Fév','Mar','Avr','Mai','Juin','Juil','Août','Sep','Oct','Nov','Déc'];const renderCard=(d,isPast)=>{const s=d.soirees;const e=d.etablissements;const dt=s?.date_soiree?new Date(s.date_soiree+'T00:00:00'):null;return(<div key={d.id} className={`bg-bg border rounded-xl p-4 flex items-center gap-4 transition ${isPast?'border-border/50 opacity-60':'border-border hover:border-dim'}`}><div className="text-center w-12 flex-shrink-0"><div className="text-lg font-semibold">{dt?.getDate()}</div><div className="text-[10px] text-dim uppercase">{dt?mn[dt.getMonth()]:''}</div></div><div className="flex-1 min-w-0"><div className="text-sm font-medium">{s?.titre}</div><div className="text-xs text-dim mt-0.5"><Link href={`/profil/${d.etablissement_id}`} className="hover:text-blue transition">{e?.nom}</Link></div><div className="text-xs text-accent mt-1">🕐 {s?.heure_debut}–{s?.heure_fin}</div></div><span className={`text-[10px] px-2.5 py-1 rounded-full font-medium border flex-shrink-0 ${isPast?'bg-dim/10 border-border text-dim':'bg-green-400/10 border-green-400/20 text-green-400'}`}>{isPast?'Passée':'Confirmé'}</span></div>);};return(<><div className="mb-5 animate-fade-up" style={{animationDelay:'0.15s'}}>{upcoming.length>0&&<><h2 className="text-sm font-medium flex items-center gap-2 mb-3">🎵 Prochaines soirées</h2><div className="space-y-2 mb-4">{upcoming.map(d=>renderCard(d,false))}</div></>}{past.length>0&&<><h2 className="text-sm font-medium flex items-center gap-2 mb-3 text-dim">📁 Soirées passées</h2><div className="space-y-2">{past.map(d=>renderCard(d,true))}</div></>}{upcoming.length===0&&past.length===0&&<div className="bg-bg border border-border rounded-xl p-6 text-center"><div className="text-3xl mb-2">🎵</div><p className="text-xs text-dim">Aucune soirée</p></div>}</div></>);})()}
  ...
  return(
    <div className="min-h-screen px-4 py-16 sm:py-20"><div className="max-w-2xl mx-auto">
      ...
      <div className="mb-5 animate-fade-up" style={{animationDelay:'0.1s'}}>
        <Link href="/soiree/creer" className="w-full flex items-center justify-center gap-2 p-3.5 border border-dashed border-dim rounded-xl text-sm text-dim hover:border-blue hover:text-blue hover:bg-blue/5 transition mb-5"><span className="text-lg">+</span> Créer une nouvelle soirée</Link>
        {loadingSoirees?<div className="flex justify-center py-6"><div className="w-5 h-5 border-2 border-blue/30 border-t-blue rounded-full animate-spin"/></div>:soirees.length===0?<div className="bg-bg border border-border rounded-xl p-6 text-center"><div className="text-3xl mb-2">📅</div><p className="text-xs text-dim">Aucune soirée</p></div>:<>
        {soirees.filter(s=>s.status==='confirmed'&&s.date_soiree>=todayVenue).length>0&&<div className="mb-4"><h2 className="text-sm font-medium flex items-center gap-2 mb-3">✅ Soirées confirmées</h2><div className="space-y-2">{soirees.filter(s=>s.status==='confirmed'&&s.date_soiree>=todayVenue).sort((a,b)=>a.date_soiree.localeCompare(b.date_soiree)).map(s=><SoireeCard key={s.id} s={s} sc={sc} formatD={formatD} deleteSoiree={deleteSoiree}/>)}</div></div>}
        {soirees.filter(s=>s.status==='draft'&&s.date_soiree>=todayVenue).length>0&&<div className="mb-4"><h2 className="text-sm font-medium flex items-center gap-2 mb-3">🔍 En recherche d&apos;artiste</h2><div className="space-y-2">{soirees.filter(s=>s.status==='draft'&&s.date_soiree>=todayVenue).sort((a,b)=>a.date_soiree.localeCompare(b.date_soiree)).map(s=><SoireeCard key={s.id} s={s} sc={sc} formatD={formatD} deleteSoiree={deleteSoiree} showActions/>)}</div></div>}
        {soirees.filter(s=>s.date_soiree<todayVenue).length>0&&<div className="mb-4"><h2 className="text-sm font-medium flex items-center gap-2 mb-3 text-dim">📁 Soirées passées</h2><div className="space-y-2">{soirees.filter(s=>s.date_soiree<todayVenue).sort((a,b)=>b.date_soiree.localeCompare(a.date_soiree)).map(s=><SoireeCard key={s.id} s={s} sc={sc} formatD={formatD} deleteSoiree={deleteSoiree} past/>)}</div></div>}
        </>}
      </div>
