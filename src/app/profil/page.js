...
function ArtistDashboard({data,user,completion,supabase}){
  ...
  useEffect(()=>{
    async function loadDemandes(){
      const{data:dems}=await supabase.from('demandes').select('*, soirees(*), etablissements:etablissement_id(nom, photos)').eq('artiste_id',user.id).order('created_at',{ascending:false});
      setDemandes(dems||[]);
  ...
  const handleDemande=async(demandeId,status)=>{
    await supabase.from('demandes').update({status}).eq('id',demandeId);
    if(status==='accepted'){
      const dem=demandes.find(d=>d.id===demandeId);
      if(dem)await supabase.from('soirees').update({status:'confirmed'}).eq('id',dem.soiree_id);
    }
    setDemandes(prev=>prev.map(d=>d.id===demandeId?{...d,status}:d));
  };
 
  const formatD=(d)=>{if(!d)return'';const dt=new Date(d);const mn=['jan','fév','mar','avr','mai','juin','juil','août','sep','oct','nov','déc'];return`${dt.getDate()} ${mn[dt.getMonth()]} ${dt.getFullYear()}`;};
  ...
  return(
    <div className="min-h-screen px-4 py-20"><div className="max-w-2xl mx-auto">
      ...
      <div className="grid grid-cols-4 gap-3 mb-5 animate-fade-up" style={{animationDelay:'0.05s'}}>
        ...
        <div className="bg-bg-card rounded-xl p-3.5 text-center"><div className="text-xl font-semibold text-blue">{demandes.length}</div><div className="text-[11px] text-dim mt-1">Total demandes</div></div>
      ...
      <div className="mb-5 animate-fade-up" style={{animationDelay:'0.07s'}}>
        ...
        {datesDispo.length===0&&!showCal&&<p className="text-xs text-dim">Aucune date — cliquez &quot;Gérer mes dates&quot;</p>}
      ...
      <div className="mb-5 animate-fade-up" style={{animationDelay:'0.1s'}}>
        ...
        {loadingDemandes?<div className="flex justify-center py-6"><div className="w-5 h-5 border-2 border-accent/30 border-t-accent rounded-full animate-spin"/></div>:
        demandes.length===0?<div className="bg-bg border border-border rounded-xl p-6 text-center"><div className="text-3xl mb-2">📭</div><p className="text-xs text-dim">Aucune demande pour le moment</p><p className="text-[10px] text-dim mt-1">Les établissements vous contacteront quand ils cherchent un artiste</p></div>:
        <div className="space-y-2">{demandes.map(d=>{const st=stMap[d.status]||stMap.pending;const s=d.soirees;const e=d.etablissements;return(
          <div key={d.id} className="bg-bg border border-border rounded-xl p-4 hover:border-dim transition">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0"><div className="text-sm font-medium">{e?.nom||'Établissement'}</div><div className="text-xs text-dim mt-0.5">{s?.titre} · {s?.ambiance||''}</div></div>
              <span className={`text-[10px] px-2.5 py-1 rounded-full font-medium border ${st.bg}`}>{st.label}</span>
            </div>
            <div className="flex items-center justify-between mt-3 pt-3 border-t border-border">
              <span className="text-xs text-dim flex items-center gap-1.5">📅 {formatD(s?.date_soiree)} · {s?.heure_debut}–{s?.heure_fin}</span>
              <div className="flex gap-2">
                {d.status==='pending'&&(<>
                  <button onClick={()=>handleDemande(d.id,'declined')} className="text-xs px-3 py-1.5 rounded-lg bg-bg-card border border-border text-muted hover:text-white transition">Refuser</button>
                  <button onClick={()=>handleDemande(d.id,'accepted')} className="text-xs px-3 py-1.5 rounded-lg bg-accent text-black font-medium">Accepter</button>
                </>)}
      ...
      {/* UPCOMING ACCEPTED */}
      {accepted.length>0&&<div className="mb-5 animate-fade-up" style={{animationDelay:'0.15s'}}>
        <h2 className="text-sm font-medium flex items-center gap-2 mb-3">🎵 Prochaines soirées</h2>
        <div className="space-y-2">{accepted.map(d=>{const s=d.soirees;const e=d.etablissements;const dt=s?.date_soiree?new Date(s.date_soiree):null;return(
          <div key={d.id} className="bg-bg border border-border rounded-xl p-4 flex items-center gap-4">
            <div className="text-center w-12 flex-shrink-0"><div className="text-lg font-semibold">{dt?.getDate()}</div><div className="text-[10px] text-dim uppercase tracking-wider">{dt?['Jan','Fév','Mar','Avr','Mai','Juin','Juil','Août','Sep','Oct','Nov','Déc'][dt.getMonth()]:''}</div></div>
            <div className="flex-1 min-w-0"><div className="text-sm font-medium">{s?.titre}</div><div className="text-xs text-dim mt-0.5">{e?.nom}</div><div className="text-xs text-accent flex items-center gap-1 mt-1">🕐 {s?.heure_debut}–{s?.heure_fin}</div></div>
            <span className="text-[10px] px-2.5 py-1 rounded-full font-medium border bg-green-400/10 border-green-400/20 text-green-400">Confirmé</span>
          </div>
        );})}</div>
      </div>}
...
function VenueDashboard({data,user,completion,supabase}){
  ...
  const[loadingSoirees,setLoadingSoirees]=useState(true);
 
  useEffect(()=>{
    async function loadSoirees(){
      const{data:soirs}=await supabase.from('soirees').select('*').eq('etablissement_id',user.id).order('date_soiree',{ascending:true});
      const soireesWithDemandes=[];
      for(const s of(soirs||[])){
        const{data:dems}=await supabase.from('demandes').select('*, artistes:artiste_id(nom_scene, type_artiste, photo_url)').eq('soiree_id',s.id);
        soireesWithDemandes.push({...s,demandes:dems||[]});
      }
      setSoirees(soireesWithDemandes);
      setLoadingSoirees(false);
    }
    loadSoirees();
  },[]);
  ...
  return(
    <div className="min-h-screen px-4 py-20"><div className="max-w-2xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-3 mb-5 animate-fade-up" style={{animationDelay:'0.05s'}}>
        <div className="bg-bg-card rounded-xl p-3.5 text-center"><div className="text-xl font-semibold text-blue">{soirees.length}</div><div className="text-[11px] text-dim mt-1">Soirées créées</div></div>
        ...
        <div className="bg-bg-card rounded-xl p-3.5 text-center"><div className="text-xl font-semibold text-accent">{soirees.reduce((n,s)=>n+s.demandes.length,0)}</div><div className="text-[11px] text-dim mt-1">Demandes envoyées</div></div>
      </div>
 
      <div className="mb-5 animate-fade-up" style={{animationDelay:'0.1s'}}>
        <div className="flex items-center justify-between mb-3"><h2 className="text-sm font-medium flex items-center gap-2">📅 Mes soirées</h2></div>
        ...
        soirees.length===0?<div className="bg-bg border border-border rounded-xl p-6 text-center"><div className="text-3xl mb-2">📅</div><p className="text-xs text-dim">Aucune soirée créée</p><Link href="/soiree/creer" className="text-xs text-blue hover:underline mt-2 inline-block">Créer votre première soirée</Link></div>:
        <div className="space-y-2">{soirees.map(s=>{const st=sc[s.status]||sc.draft;return(
          <div key={s.id} className="bg-bg border border-border rounded-xl p-4 hover:border-dim transition">
            ...
            <div className="flex items-center justify-between pt-3 border-t border-border"><span className="text-xs text-dim">{s.ambiance&&`🎵 ${s.ambiance}`}</span><div className="flex gap-2">{s.status==='draft'&&<Link href="/explorer" className="text-xs px-3 py-1.5 rounded-lg bg-blue text-black font-medium">Trouver un artiste</Link>}</div></div>
          </div>
        );})}</div>}
      </div>
