...
function Calendar({ selectedDates=[], onToggleDate, color='accent' }) {
      ...
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-8 animate-fade-up">
          <h1 className="font-display text-3xl md:text-4xl font-bold tracking-tight">
            {isArtistView ? 'Explorer les artistes' : 'Explorer les établissements'}
          </h1>
          <p className="text-sm text-muted mt-2">
            {isArtistView ? "Trouvez l'artiste idéal pour votre prochaine soirée" : 'Trouvez des lieux où jouer près de chez vous'}
          </p>
        </div>
        ...
        <div className="mb-5 animate-fade-up" style={{animationDelay:'0.05s'}}>
          <div className="relative">
            ...
            <input value={search} onChange={(e)=>setSearch(e.target.value)} className="w-full bg-bg-card border border-border rounded-xl pl-12 pr-4 py-3.5 text-sm text-white focus:border-accent transition placeholder:text-dim"
              placeholder={isArtistView ? 'Rechercher par nom, ville, style...' : 'Rechercher par nom, ville, ambiance...'} />
        ...
        {showCalendar && <div className="mb-5 max-w-sm animate-fade-up"><Calendar selectedDates={dateFilter} onToggleDate={toggleDate} color="accent" /><p className="text-[10px] text-dim mt-2">Cliquez sur les dates pour filtrer</p></div>}
        ...
        {loading ? (
          ...
          <div className="text-center py-20 animate-fade-up"><div className="text-5xl mb-4">{isArtistView?'🎵':'🏪'}</div><h3 className="font-display text-lg font-semibold mb-2">Aucun résultat</h3><p className="text-sm text-muted mb-4">Essayez avec d&apos;autres filtres</p><button onClick={()=>{setSearch('');setTypeFilter('');setStyleFilter('');setVilleFilter('');setDispoFilter('');setDateFilter([]);}} className="text-sm text-accent hover:underline">Tout afficher</button></div>
        ) : (
    ...
    <div className="bg-bg-card border border-border rounded-2xl p-5 hover:border-accent/20 hover:-translate-y-0.5 transition animate-fade-up group" style={{animationDelay:`${Math.min(i*0.05,0.5)}s`}}>
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {a.styles?.length>0 && <div className="flex flex-wrap gap-1.5 mt-2.5">{a.styles.slice(0,5).map(s=>(<span key={s} onClick={()=>setStyleFilter(s)} className={`text-[10px] px-2.5 py-0.5 rounded-full cursor-pointer transition ${styleFilter===s?'bg-accent/20 border border-accent/40 text-accent font-medium':'bg-accent/10 border border-accent/15 text-accent hover:bg-accent/15'}`}>{s}</span>))}{a.styles.length>5&&<span className="text-[10px] px-2.5 py-0.5 rounded-full bg-bg-hover text-dim">+{a.styles.length-5}</span>}</div>}
        </div>
      </div>
...
function VenueCard({ v, i }) {
  const eqMap = {};
  EQUIPEMENTS.forEach(e => { eqMap[e.id] = e; });
  return (
    <div className="bg-bg-card border border-border rounded-2xl overflow-hidden hover:border-blue/20 hover:-translate-y-0.5 transition animate-fade-up" style={{animationDelay:`${Math.min(i*0.05,0.5)}s`}}>
      {v.photos?.length > 0 ? (
        <div className="h-32 overflow-hidden"><img src={v.photos[0]} alt={v.nom} className="w-full h-full object-cover" /></div>
      ) : (
        <div className="h-24 bg-gradient-to-br from-blue/10 to-bg-mid" />
      )}
      <div className="p-5">
        ...
        {v.equipements?.length > 0 && <div className="flex flex-wrap gap-1.5 mt-2">{v.equipements.map(eq=>{const e=eqMap[eq];return <span key={eq} className="text-[10px] px-2 py-0.5 rounded-full bg-bg-hover text-dim">{e?.icon} {e?.label||eq}</span>;})}</div>}
        {v.site_web && <a href={v.site_web.startsWith('http')?v.site_web:`https://${v.site_web}`} target="_blank" className="text-xs text-blue hover:underline mt-2 inline-block">🌐 {v.site_web}</a>}
