...
export default function Explorer() {
  ...
  const formatDate = (d) => { const [yy, mm, dd] = d.split('-'); const mn = ['jan', 'fév', 'mar', 'avr', 'mai', 'juin', 'juil', 'août', 'sep', 'oct', 'nov', 'déc']; return `${parseInt(dd)} ${mn[parseInt(mm) - 1]}`; };
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-5 animate-fade-up" style={{ animationDelay: '0.05s' }}>
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-bg-card border border-border rounded-xl pl-12 pr-4 py-3.5 text-sm text-white focus:border-accent transition placeholder:text-dim" placeholder={isArtistView ? 'Rechercher par nom, ville, style...' : 'Rechercher par nom, ville...'} />
        ...
        <div className="mb-4 animate-fade-up" style={{ animationDelay: '0.1s' }}>
          <div className="flex gap-2 overflow-x-auto pb-1">
            ...
            {isArtistView
              ...
              : VENUE_TYPES.map(t => (<button key={t} onClick={() => setTypeFilter(t)} className={`chip whitespace-nowrap flex-shrink-0 ${typeFilter === t ? 'active' : ''}`}>{t}</button>))}
          </div>
        </div>
 
        {isArtistView && availableStyles.length > 0 && (
          <div className="mb-4 animate-fade-up" style={{ animationDelay: '0.12s' }}>
            <div className="flex gap-2 overflow-x-auto pb-1">
              <button onClick={() => setStyleFilter('')} className={`text-xs px-3 py-1.5 rounded-full border whitespace-nowrap flex-shrink-0 transition ${!styleFilter ? 'bg-accent/10 border-accent/30 text-accent font-medium' : 'bg-bg-card border-border text-dim'}`}>Tous styles</button>
              {availableStyles.slice(0, 12).map(s => (<button key={s} onClick={() => setStyleFilter(styleFilter === s ? '' : s)} className={`text-xs px-3 py-1.5 rounded-full border whitespace-nowrap flex-shrink-0 transition ${styleFilter === s ? 'bg-accent/10 border-accent/30 text-accent font-medium' : 'bg-bg-card border-border text-dim'}`}>{s}</button>))}
            </div>
          </div>
        )}
 
        <div className="flex flex-wrap gap-3 mb-4 animate-fade-up" style={{ animationDelay: '0.14s' }}>
          {isArtistView && <button onClick={() => setShowCalendar(!showCalendar)} className={`text-xs px-4 py-2 rounded-lg border flex items-center gap-2 transition ${dateFilter.length > 0 || showCalendar ? 'bg-accent/10 border-accent/30 text-accent font-medium' : 'bg-bg-card border-border text-muted'}`}>📅 {dateFilter.length > 0 ? `${dateFilter.length} date${dateFilter.length > 1 ? 's' : ''}` : 'Filtrer par date'}</button>}
          {allVilles.length > 1 && <select value={villeFilter} onChange={e => setVilleFilter(e.target.value)} className="bg-bg-card border border-border rounded-lg px-3 py-2 text-xs text-muted appearance-none cursor-pointer"><option value="">Toutes les villes</option>{allVilles.map(v => (<option key={v} value={v}>{v}</option>))}</select>}
          {isArtistView && <select value={dispoFilter} onChange={e => setDispoFilter(e.target.value)} className="bg-bg-card border border-border rounded-lg px-3 py-2 text-xs text-muted appearance-none cursor-pointer"><option value="">Tous les jours</option>{['Lun', 'Mar', 'Mer', 'Jeu', 'Ven', 'Sam', 'Dim'].map(j => (<option key={j} value={j}>{j}</option>))}</select>}
          {(search || styleFilter || villeFilter || dispoFilter || dateFilter.length > 0 || typeFilter) && <button onClick={() => { setSearch(''); setStyleFilter(''); setVilleFilter(''); setDispoFilter(''); setDateFilter([]); setShowCalendar(false); setTypeFilter(''); }} className="text-xs text-accent hover:underline">Réinitialiser</button>}
        </div>
        ...
        {showCalendar && <div className="mb-5 max-w-sm animate-fade-up"><Calendar selectedDates={dateFilter} onToggleDate={toggleDate} /></div>}
 
        <div className="mb-4"><span className="text-xs text-dim">{loading ? 'Chargement...' : `${filtered.length} résultat${filtered.length !== 1 ? 's' : ''}`}</span></div>
...
function ArtistCard({ a, i, styleFilter, setStyleFilter, dateFilter, soirees, supabase, userId, existingDemandes, addDemande }) {
  ...
  const availableSoirees = (soirees || []).filter(s => !(existingDemandes || []).some(d => d.soiree_id === s.id && d.artiste_id === a.id));
  const alreadyProposed = (existingDemandes || []).some(d => d.artiste_id === a.id);
 
  const sendDemande = async () => {
    if (!selectedSoiree) return;
    setSending(true);
    const { error } = await supabase.from('demandes').insert({ soiree_id: selectedSoiree, etablissement_id: userId, artiste_id: a.id, status: 'pending', message: message.trim() || null });
    setSending(false);
    if (!error) {
      setProposeSent(true);
      setShowPropose(false);
      setMessage('');
      addDemande({ soiree_id: selectedSoiree, artiste_id: a.id, status: 'pending' });
    ...
    <div className="bg-bg-card border border-border rounded-2xl p-5 hover:border-accent/20 transition animate-fade-up" style={{ animationDelay: `${Math.min(i * 0.05, 0.5)}s` }}>
      ...
      <div className="flex items-center justify-between mt-4 pt-3 border-t border-border">
        ...
        <div className="flex items-center gap-1.5">
          {a.soundcloud && <a href={a.soundcloud.startsWith('http') ? a.soundcloud : `https://${a.soundcloud}`} target="_blank" className="w-7 h-7 rounded-md bg-orange-500/10 flex items-center justify-center text-xs">☁️</a>}
          {a.instagram && <a href={`https://instagram.com/${a.instagram.replace('@', '')}`} target="_blank" className="w-7 h-7 rounded-md bg-pink-500/10 flex items-center justify-center text-xs">📸</a>}
        </div>
      </div>
      ...
      <div className="flex gap-2 mt-3">
        ...
        {!proposeSent && availableSoirees.length === 0 && alreadyProposed && <Link href="/soiree/creer" className="flex-1 text-xs py-2.5 rounded-lg bg-blue/10 text-blue border border-blue/20 text-center hover:bg-blue/20 transition">+ Créer une soirée à proposer</Link>}
      </div>
 
      {showPropose && (
        <div className="mt-3 bg-bg border border-blue/20 rounded-xl p-4 animate-fade-up space-y-3">
          <div className="text-xs font-medium text-blue">📋 Choisissez une soirée pour {a.nom_scene}</div>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={sendDemande} disabled={!selectedSoiree || sending} className="flex-1 text-xs py-2 rounded-lg bg-blue text-black font-medium disabled:opacity-30 transition">{sending ? 'Envoi...' : '📨 Envoyer la demande'}</button>
          </div>
        </div>
      )}
    </div>
