...
export default function OnboardingArtiste() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: existing } = await supabase.from('artistes').select('*').eq('id', user.id).single();
      if (existing) {
        setIsEdit(true);
        setNomScene(existing.nom_scene || '');
        setVille(existing.ville || '');
        setTypeArtiste(existing.type_artiste || '');
        setStyles(existing.styles || []);
        setBio(existing.bio || '');
        setPhotoPreview(existing.photo_url || '');
        setSoundcloud(existing.soundcloud || '');
        setInstagram(existing.instagram || '');
        setTiktok(existing.tiktok || '');
        setYoutube(existing.youtube || '');
        setSpotify(existing.spotify || '');
        setDispos(existing.disponibilites || []);
      }
  ...
  const handleSubmit = async () => {
    ...
    await supabase.from('artistes').upsert({
      id: user.id,
      nom_scene: nomScene, ville, type_artiste: typeArtiste,
      styles, bio, photo_url: photoUrl,
    ...
    <div className="min-h-screen px-4 py-20">
      <div className="max-w-lg mx-auto animate-fade-up">
        <div className="text-center mb-8">
          ...
          <h1 className="font-display text-2xl font-bold">{isEdit ? 'Modifier mon profil' : 'Créez votre profil artiste'}</h1>
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={nomScene} onChange={(e) => setNomScene(e.target.value)} required
                    className="w-full bg-bg-mid border border-border rounded-lg px-3 py-2.5 text-sm text-white focus:border-accent transition" placeholder="Ex : DJ Milo" />
                </div>
                <div>
                  ...
                  <input value={ville} onChange={(e) => setVille(e.target.value)} required
                    className="w-full bg-bg-mid border border-border rounded-lg px-3 py-2.5 text-sm text-white focus:border-accent transition" placeholder="Ex : Cannes" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {ARTIST_TYPES.map(t => (
                    <button key={t} type="button" onClick={() => { setTypeArtiste(t); if (typeArtiste !== t) setStyles([]); }}
                      className={`chip ${typeArtiste === t ? 'active' : ''}`}>{ARTIST_EMOJIS[t]} {t}</button>
                  ))}
                </div>
              </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {availableStyles.map(s => (
                      <button key={s} type="button" onClick={() => toggleStyle(s)}
                        className={`chip text-xs ${styles.includes(s) ? 'active' : ''}`}>{s}</button>
                    ))}
                  </div>
                </div>
              )}
              ...
              <button onClick={() => setStep(2)} disabled={!nomScene || !ville || !typeArtiste}
                className="w-full bg-accent text-black font-bold py-3 rounded-xl hover:shadow-lg hover:shadow-accent/25 transition disabled:opacity-30">Suivant</button>
            </div>
          )}
            ...
            <div className="space-y-5">
                ...
                <div className="grid grid-cols-7 gap-2">
                  {JOURS.map(j => (
                    <button key={j} type="button" onClick={() => toggleDispo(j)}
                      className={`py-3 rounded-lg text-center text-xs font-medium transition ${dispos.includes(j) ? 'bg-accent/10 border border-accent/30 text-accent' : 'bg-bg-mid border border-border text-dim hover:border-dim'}`}>{j}</button>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex bg-bg-mid border border-border rounded-lg overflow-hidden focus-within:border-accent transition">
                  <div className="px-3 py-2.5 border-r border-border flex items-center bg-orange-500/5"><span className="text-orange-500 text-sm">☁️</span></div>
                  <input value={soundcloud} onChange={(e) => setSoundcloud(e.target.value)}
                    className="flex-1 bg-transparent px-3 py-2.5 text-sm text-white outline-none" placeholder="soundcloud.com/ton-profil" />
                </div>
              </div>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <div key={s.key} className="flex bg-bg-mid border border-border rounded-lg overflow-hidden focus-within:border-accent transition">
                      <div className={`px-3 py-2.5 border-r border-border flex items-center ${s.color}`}><span className="text-sm">{s.icon}</span></div>
                      <input value={s.val} onChange={(e) => s.set(e.target.value)}
                        className="flex-1 bg-transparent px-3 py-2.5 text-sm text-white outline-none" placeholder={s.placeholder} />
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="flex gap-3">
                <button onClick={() => setStep(1)} className="px-6 py-3 rounded-xl border border-border text-sm text-muted hover:bg-bg-hover transition">Retour</button>
                <button onClick={handleSubmit} disabled={loading}
                  ...
                  {loading ? 'Enregistrement...' : isEdit ? '✅ Enregistrer les modifications' : '✨ Publier mon profil'}
                </button>
              </div>
            </div>
          )}
