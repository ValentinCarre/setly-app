...
function Calendar({ selectedDates=[], onToggleDate, color='blue' }) {
  ...
  const ip=(d)=>new Date(y,m,d)<t;
  const it=(d)=>y===t.getFullYear()&&m===t.getMonth()&&d===t.getDate();
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center animate-fade-up">
          ...
          <h2 className="font-display text-xl font-bold mb-2">{dateSoiree.length > 1 ? `${dateSoiree.length} soirées créées !` : 'Soirée créée !'}</h2>
    ...
    <div className="min-h-screen px-4 py-20">
      <div className="max-w-lg mx-auto animate-fade-up">
        <div className="text-center mb-8">
          ...
          <h1 className="font-display text-2xl font-bold">Créer une soirée</h1>
          <p className="text-sm text-dim mt-1">Planifiez votre événement et trouvez un artiste</p>
        </div>
 
        <div className="bg-bg-card border border-border rounded-2xl p-6 space-y-5">
          <div>
            <label className="text-xs text-dim uppercase tracking-wider font-medium block mb-1.5">Nom de la soirée *</label>
            <input value={titre} onChange={(e) => setTitre(e.target.value)} className="w-full bg-bg-mid border border-border rounded-lg px-4 py-3 text-sm text-white focus:border-blue transition" placeholder="Ex : Soirée Jazz Terrasse" />
          </div>
 
          <div>
            <label className="text-xs text-dim uppercase tracking-wider font-medium block mb-2">Date(s) de la soirée *</label>
            <Calendar selectedDates={dateSoiree} onToggleDate={toggleDate} color="blue" />
            {dateSoiree.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-3">
                {dateSoiree.sort().map(d => (
                  <span key={d} className="text-xs px-3 py-1 rounded-full bg-blue/10 border border-blue/20 text-blue flex items-center gap-1.5">
                    {formatD(d)} <button onClick={() => toggleDate(d)} className="hover:text-white">✕</button>
                  </span>
                ))}
              </div>
            )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-dim uppercase tracking-wider font-medium block mb-1.5">Heure de début</label>
              <input type="time" value={heureDebut} onChange={(e) => setHeureDebut(e.target.value)} className="w-full bg-bg-mid border border-border rounded-lg px-4 py-3 text-sm text-white focus:border-blue transition" />
            </div>
            <div>
              <label className="text-xs text-dim uppercase tracking-wider font-medium block mb-1.5">Heure de fin</label>
              <input type="time" value={heureFin} onChange={(e) => setHeureFin(e.target.value)} className="w-full bg-bg-mid border border-border rounded-lg px-4 py-3 text-sm text-white focus:border-blue transition" />
            </div>
          </div>
 
          <div>
            <label className="text-xs text-dim uppercase tracking-wider font-medium block mb-2">Ambiance recherchée</label>
            <div className="flex flex-wrap gap-2">
              {AMBIANCES.map(a => (
                <button key={a} onClick={() => setAmbiance(ambiance === a ? '' : a)} className={`chip ${ambiance === a ? 'active-blue' : ''}`}>{a}</button>
              ))}
            </div>
          </div>
 
          <div>
            <label className="text-xs text-dim uppercase tracking-wider font-medium block mb-1.5">Description (optionnel)</label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="w-full bg-bg-mid border border-border rounded-lg px-4 py-3 text-sm text-white focus:border-blue transition resize-none" placeholder="Décrivez l'ambiance souhaitée, le type de public..." />
          </div>
