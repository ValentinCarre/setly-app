    ...
    <div className="min-h-screen px-4 py-20">
      <div className="max-w-lg mx-auto animate-fade-up">
        ...
        <div className="bg-bg-card border border-border rounded-2xl p-6 space-y-5">
          ...
          {/* NB ARTISTES */}
          <div>
            <label className="text-xs text-dim uppercase tracking-wider font-medium block mb-2">Nombre d'artistes souhaités</label>
            <div className="flex items-center gap-3">
              <button type="button" onClick={() => setNbArtistes(Math.max(1, nbArtistes - 1))} className="w-10 h-10 rounded-lg bg-bg-mid border border-border flex items-center justify-center text-lg text-muted hover:text-white hover:border-dim transition">−</button>
              <div className="text-center min-w-[60px]"><div className="text-2xl font-semibold text-blue">{nbArtistes}</div><div className="text-[10px] text-dim">artiste{nbArtistes > 1 ? "s" : ""}</div></div>
              <button type="button" onClick={() => setNbArtistes(Math.min(10, nbArtistes + 1))} className="w-10 h-10 rounded-lg bg-bg-mid border border-border flex items-center justify-center text-lg text-muted hover:text-white hover:border-dim transition">+</button>
            </div>
          </div>
